...
bool check_leaks()
{
	...
	for (int i = 0; i < DEBUG_NEW_HASHTABLESIZE; ++i)
	{
		...
		while (ptr)
		{
			printf("Leaked object at %p (size %lu, %s:%d)\n",
...
void* operator new(size_t size, const char* file, int line)
{
	...
	if (ptr == NULL)
	{
		fprintf(stderr, "new:  out of memory when allocating %lu bytes\n",
				size);
		abort();
	}
	...
	if (new_verbose_flag)
		printf("new:  allocated  %p (size %lu, %s:%d)\n",
...
void operator delete(void* pointer)
{
	...
	while (ptr)
	{
		...
		{
			if (new_verbose_flag)
				printf("delete: freeing  %p (size %lu)\n", pointer, ptr->size);
			if (ptr_last == NULL)
